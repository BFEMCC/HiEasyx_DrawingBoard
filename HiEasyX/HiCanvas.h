...
*2023年6月5日22时 完善功能，添加参数：窗口对象指针。在调用该函数后无需再次调用clear()设置背景色，调用该函数，传入相应参数即可成功设置背景色
				  同时需要添加#include "HiWindow.h"头文件
*/
/*****************************************************************************************************/
		void InItAndBindWindow(int w, int h, COLORREF bkcolor = WHITE, Window* wnd = nullptr)
		{
			CleanUpSettings();
			Resize(w, h);
			SetBkColor(bkcolor);
